...
def staff_attendance_percentage_this_week(staff):
    ...
    qs2 = Present.objects.filter(date__gte=first_day, date__lte=last_day, user=staff)
    total_days = len(qs2)
    if total_days == 0:
        return 0.0  # or any other appropriate value to handle the case when there are no days
