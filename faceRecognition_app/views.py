    ...
    if(qs.is_sampleUploaded==True):
        ...
        while (True):
            ...
            for (x, y, w, h) in faces:
                ...
                cv.imwrite("faceRecognition_data/training_dataset/" + Name + "/" + str(ID) + "." + str(sampleNum) + ".jpg",
                                    gray[y:y + h, x:x + w])
...
def train_data(request):
    ...
    if not qs.is_sampleUploaded:
        ...
        # Modify the training dataset path here if needed
        training_dataset_path = "faceRecognition_data/training_dataset"
        faces, Id = getImagesAndLabels(training_dataset_path, face_cascade)

        recognizer.train(faces, np.array(Id))
        try:
            # Modify the trained model save path here if needed
            trained_model_path = "model/trained_model2.yml"
            recognizer.save(trained_model_path)
        except Exception as e:
            q = 'Please create a "model" folder'
...
def getImagesAndLabels(path, face_cascade):
    ...
    for imagePath in imagePaths:
        ...
        faces = face_cascade.detectMultiScale(imageNp, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30))
        for (x, y, w, h) in faces:
...
def attendance_out(request):
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if confidence > 77:
                qs = Dataset.objects.filter(sample=str(id)).first()
                if qs:
                    id_ = qs.user.username
                    confidence = "  {0}%".format(round(100 - confidence))
                    present[id_] = True
...
def update_attendance_in_db_out(present):
    ...
    for person in present:
        try:
            user = User.objects.get(username=person)
            if present[person] and (Present.objects.filter(user=user).filter(date=today).filter(present=True).exists() or Time.objects.filter(user=user).filter(date=today).filter(out=False).exists()):
...
#             # Close the camera window if a face is detected
#             cv.destroyAllWindows()
#             break
